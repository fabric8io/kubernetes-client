...
type EgressNetworkPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*network.EgressNetworkPolicy, err error)
...
// List lists all EgressNetworkPolicies in the indexer.
func (s *egressNetworkPolicyLister) List(selector labels.Selector) (ret []*network.EgressNetworkPolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*network.EgressNetworkPolicy))
	})
	return ret, err
}
...
type EgressNetworkPolicyNamespaceLister interface {
	...
	Get(name string) (*network.EgressNetworkPolicy, error)
...
// List lists all EgressNetworkPolicies in the indexer for a given namespace.
func (s egressNetworkPolicyNamespaceLister) List(selector labels.Selector) (ret []*network.EgressNetworkPolicy, err error) {
...
// Get retrieves the EgressNetworkPolicy from the indexer for a given namespace and name.
func (s egressNetworkPolicyNamespaceLister) Get(name string) (*network.EgressNetworkPolicy, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(network.Resource("egressnetworkpolicy"), name)
	}
	return obj.(*network.EgressNetworkPolicy), nil
}
