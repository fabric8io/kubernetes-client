...
public class CounterSet implements Editable<CounterSetBuilder>, KubernetesResource
{
    ...
    /**
     * Counters defines the counters that will be consumed by the device. The name of each counter must be unique in that set and must be a DNS label.<br><p> <br><p> To ensure this uniqueness, capacities defined by the vendor must be listed without the driver name as domain prefix in their name. All others must be listed with their domain prefix.<br><p> <br><p> The maximum number of counters is 32.
     */
    @JsonProperty("counters")
    @JsonInclude(JsonInclude.Include.NON_EMPTY)
