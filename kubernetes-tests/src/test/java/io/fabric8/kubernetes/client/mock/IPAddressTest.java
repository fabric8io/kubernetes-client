...
import io.fabric8.kubernetes.api.model.networking.v1.IPAddressListBuilder;
...
class IPAddressTest {
  ...
  void get() {
    // Given
    server.expect().get().withPath("/apis/networking.k8s.io/v1/ipaddresses/192.168.1.5")
        .andReturn(HttpURLConnection.HTTP_OK, createNewIPAddress())
        .once();

    // When
    IPAddress ipAddress = client.network().v1().ipAddresses().withName("192.168.1.5").get();
  ...
  void list() {
    // Given
    server.expect().get().withPath("/apis/networking.k8s.io/v1/ipaddresses")
    ...
    // When
    IPAddressList ipAddressList = client.network().v1().ipAddresses().list();
  ...
  void delete() {
    // Given
    server.expect().delete().withPath("/apis/networking.k8s.io/v1/ipaddresses/192.168.1.5")
        .andReturn(HttpURLConnection.HTTP_OK, createNewIPAddress())
        .once();

    // When
    boolean isDeleted = client.network().v1().ipAddresses().withName("192.168.1.5").delete()
        .size() == 1;
