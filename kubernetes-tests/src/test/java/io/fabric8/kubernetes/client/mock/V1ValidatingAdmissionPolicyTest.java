...
class V1ValidatingAdmissionPolicyTest {
  ...
  @Test
  void load() {
    List<HasMetadata> items = client.load(getClass().getResourceAsStream("/test-v1-validatingadmissionpolicy.yml"))
        .items();
  ...
  void get() {
    // Given
    server.expect().get()
        .withPath("/apis/admissionregistration.k8s.io/v1/validatingadmissionpolicies/demo-policy.example.com")
        .andReturn(HttpURLConnection.HTTP_OK, createValidatingAdmissionPolicy())
        .once();

    // When
    ValidatingAdmissionPolicy validatingAdmissionPolicy = client.admissionRegistration().v1()
  ...
  void list() {
    // Given
    server.expect().get().withPath("/apis/admissionregistration.k8s.io/v1/validatingadmissionpolicies")
    ...
    // When
    ValidatingAdmissionPolicyList flowSchemas = client.admissionRegistration().v1().validatingAdmissionPolicies().list();
  ...
  void create() {
    ...
    server.expect().post().withPath("/apis/admissionregistration.k8s.io/v1/validatingadmissionpolicies")
        .andReturn(HttpURLConnection.HTTP_OK, validatingAdmissionPolicy)
        .once();

    // When
    ValidatingAdmissionPolicy createdValidatingAdmissionPolicy = client.admissionRegistration().v1()
