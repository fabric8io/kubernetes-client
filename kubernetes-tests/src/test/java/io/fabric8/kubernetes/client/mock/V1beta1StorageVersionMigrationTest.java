...
import io.fabric8.kubernetes.api.model.storagemigration.v1beta1.StorageVersionMigrationBuilder;
...
class V1beta1StorageVersionMigrationTest {
  ...
  void storageVersionMigrations() throws InterruptedException {
    // Given
    client.storageMigration().v1beta1().storageVersionMigrations().resource(new StorageVersionMigrationBuilder()
        .withNewMetadata().withName("migrate-configmaps").endMetadata()
        .withNewSpec().withNewResource("", "configmaps").endSpec()
        .build()).create();

    // When
    final StorageVersionMigration result = client.storageMigration().v1beta1().storageVersionMigrations()
        .withName("migrate-configmaps").get();
