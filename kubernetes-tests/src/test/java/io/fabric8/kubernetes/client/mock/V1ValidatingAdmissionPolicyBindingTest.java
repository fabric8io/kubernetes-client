...
class V1ValidatingAdmissionPolicyBindingTest {
  ...
  @Test
  void load() {
    List<HasMetadata> items = client.load(getClass().getResourceAsStream("/test-v1-validatingadmissionpolicybinding.yml"))
        .items();
  ...
  void get() {
    // Given
    server.expect().get()
        .withPath("/apis/admissionregistration.k8s.io/v1/validatingadmissionpolicybindings/demo-binding-test.example.com")
        .andReturn(HttpURLConnection.HTTP_OK, createValidatingAdmissionPolicyBinding())
        .once();

    // When
    ValidatingAdmissionPolicyBinding validatingAdmissionPolicyBinding = client.admissionRegistration().v1()
  ...
  void list() {
    // Given
    server.expect().get().withPath("/apis/admissionregistration.k8s.io/v1/validatingadmissionpolicybindings")
    ...
    // When
    ValidatingAdmissionPolicyBindingList flowSchemas = client.admissionRegistration().v1()
  ...
  void create() {
    ...
    server.expect().post().withPath("/apis/admissionregistration.k8s.io/v1/validatingadmissionpolicybindings")
        .andReturn(HttpURLConnection.HTTP_OK, validatingAdmissionPolicyBinding)
        .once();

    // When
    ValidatingAdmissionPolicyBinding createdValidatingAdmissionPolicyBinding = client.admissionRegistration().v1()
