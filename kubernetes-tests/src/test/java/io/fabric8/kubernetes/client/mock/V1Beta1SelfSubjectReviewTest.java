...
import io.fabric8.kubernetes.api.model.authentication.v1beta1.SelfSubjectReviewBuilder;
...
class V1Beta1SelfSubjectReviewTest {
  ...
  void create_whenInvoked_shouldReturnObjectWithUpdatedStatus() {
    ...
    server.expect().post()
        .withPath("/apis/authentication.k8s.io/v1beta1/selfsubjectreviews")
        .andReturn(HTTP_CREATED, createNewSelfSubjectReview(Arrays.asList("system:masters", "system:authenticated")))
        .once();

    // When
    SelfSubjectReview result = client.authentication().v1beta1().selfSubjectReview().create(ssr);
