...
import io.fabric8.kubernetes.api.model.flowcontrol.v1beta3.FlowSchemaBuilder;
import io.fabric8.kubernetes.api.model.flowcontrol.v1beta3.FlowSchemaList;
import io.fabric8.kubernetes.api.model.flowcontrol.v1beta3.FlowSchemaListBuilder;
...
class V1beta3FlowSchemaTest {
  ...
  @Test
  void load() {
    List<HasMetadata> items = client.load(getClass().getResourceAsStream("/v1beta3-flowschema.yml")).items();
  ...
  void get() {
    // Given
    server.expect().get().withPath("/apis/flowcontrol.apiserver.k8s.io/v1beta3/flowschemas/exempt")
        .andReturn(HttpURLConnection.HTTP_OK, createFlowSchema("exempt"))
        .once();

    // When
    FlowSchema flowSchema = client.flowControl().v1beta3().flowSchema().withName("exempt").get();
  ...
  void list() {
    // Given
    server.expect().get().withPath("/apis/flowcontrol.apiserver.k8s.io/v1beta3/flowschemas")
    ...
    // When
    FlowSchemaList flowSchemas = client.flowControl().v1beta3().flowSchema().list();
  ...
  void create() {
    ...
    server.expect().post().withPath("/apis/flowcontrol.apiserver.k8s.io/v1beta3/flowschemas")
        .andReturn(HttpURLConnection.HTTP_OK, flowSchema)
        .once();

    // When
    FlowSchema createdFlowSchema = client.flowControl().v1beta3().flowSchema().resource(flowSchema).create();
  ...
  void delete() {
    // Given
    FlowSchema flowSchema = createFlowSchema("flowschema1");
    server.expect().delete().withPath("/apis/flowcontrol.apiserver.k8s.io/v1beta3/flowschemas/flowschema1")
        .andReturn(HttpURLConnection.HTTP_OK, flowSchema)
        .once();
