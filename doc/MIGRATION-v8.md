...
- **`JettyHttpClientFactory#additionalConfig(HttpClient, WebSocketClient)`** keeps its signature but takes Jetty 12 types. The `WebSocketClient` now runs on the given `HttpClient`, so settings (and the cookie store) apply to both, and proxy or authentication settings you added to both (the workaround for [#8029](https://github.com/fabric8io/kubernetes-client/issues/8029)) are now registered twice.
