...
Two entry points without a version in their path still used `v1beta1` APIs that Kubernetes no longer serves. They now use the `v1` models:
...
- **Cookies:** the `HttpClient` no longer keeps cookies (`HttpCookieStore.Empty`), like the other HttpClient implementations, so cookies set by a response aren't sent back on later requests or WebSocket upgrades. If you rely on them, for example for a sticky session through a load balancer, set a store in `additionalConfig`: `httpClient.setHttpCookieStore(new HttpCookieStore.Default())`.
