...
Read this file when hunting for vulnerabilities, validating security issues, or
preparing a submission.

## Scope and Severity

Consult `doc/specs/security/THREAT_MODEL.md` to verify that the vulnerability
is in scope. Start with its `Surface Index`, then read the matching surface
section before assigning severity.
...
- Current PRs/issues from this campaign. Do not reopen a report already covered
  by local findings unless the new evidence changes scope, severity,
  bypassability, or the fix.

## POC

When reporting vulnerabilities always provide proof of concept (POC).

POC should be based on Java code that demonstrates vulnerable usage of
kubernetes-client APIs.

POC can include supporting files like YAML configs or shell scripts for setting
up environment.
...
Patch should not change signature of public methods and APIs unless strictly
necessary.
