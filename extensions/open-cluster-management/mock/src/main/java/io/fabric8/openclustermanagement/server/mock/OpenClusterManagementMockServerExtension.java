 ...
 * by annotating it with <code>@ExtendWith(OpenClusterManagementServerExtension.class)</code> or through
 * <code>@EnableOpenClusterManagementMockClient</code> annotation
 */
public class OpenClusterManagementMockServerExtension extends KubernetesMockServerExtension {
  ...
  protected void destroy() {
    openClusterManagementMockServer.destroy();
    openClusterManagementClient.close();
  }
  ...
  protected void initializeKubernetesClientAndMockServer(Class<?> testClass) {
    ...
    openClusterManagementMockServer = a.crud()
      ? new OpenClusterManagementMockServer(new Context(), new MockWebServer(), responses, new KubernetesMixedDispatcher(responses), a.https())
      : new OpenClusterManagementMockServer(a.https());
    openClusterManagementMockServer.init();
    openClusterManagementClient = openClusterManagementMockServer.createOpenClusterManagement();
  }
  ...
  protected void setFieldIfKubernetesClientOrMockServer(ExtensionContext context, boolean isStatic, Field field) throws IllegalAccessException {
    setFieldIfEqualsToProvidedType(context, isStatic, field, getClientType(), (i, f) -> f.set(i, openClusterManagementClient));
    setFieldIfEqualsToProvidedType(context, isStatic, field, getKubernetesMockServerType(), (i, f) -> f.set(i, openClusterManagementMockServer));
  }
}
