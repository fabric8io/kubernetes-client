...
class PortForwarderWebsocketListenerTest {
  ...
  void onOpen_shouldPipeInChannelToWebSocket() {
    listener = new PortForwarderWebsocketListener(in, out, ForkJoinPool.commonPool());
  ...
  void onOpen_withException_shouldCloseWebSocketAndStoreException() throws IOException {
    ...
    listener = new PortForwarderWebsocketListener(inWithException, out, ForkJoinPool.commonPool());
    listener.onOpen(webSocket);
    // Then
