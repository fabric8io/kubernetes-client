...
public class MockDoneableBuild extends BuildFluentImpl<MockDoneableBuild> implements MockDoneable<Build> {
  ...
  private final Visitor<Build> visitor = new Visitor<Build>() {
    @Override
    public void visit(Build pod) {
    }
  };

  private final DoneableBuild delegate;

  public MockDoneableBuild() {
    this.delegate = EasyMock.createMock(DoneableBuild.class);
  }
  ...
  public Doneable<Build> getDelegate() {
    return new DoneableBuild(visitor) {
      @Override
      public Build done() {
