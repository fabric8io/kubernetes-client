...
public class OAuthAccessTokenHandler implements ResourceHandler<OAuthAccessToken> {
  ...
  @Override
  public Boolean delete(Client client, String namespace, OAuthAccessToken item) {
    try (OpenShiftClient osClient = client.adapt(OpenShiftClient.class)) {
      return new OAuthAccessTokenOperationsImpl(osClient, namespace, null, true, item).delete(item);
    }
  }
}
