...
public class UserHandler implements ResourceHandler<User, UserBuilder> {
  ...
  public User waitUntilCondition(OkHttpClient client, Config config, String namespace, User item, Predicate<User> condition, long amount, TimeUnit timeUnit) throws InterruptedException {
    return new UserOperationsImpl(client, OpenShiftConfig.wrap(config), null, namespace, null, true, item, null, false, -1, new TreeMap<String, String>(), new TreeMap<String, String>(), new TreeMap<String, String[]>(), new TreeMap<String, String[]>(), new TreeMap<String, String>()).waitUntilCondition(condition, amount, timeUnit);
  }
}
