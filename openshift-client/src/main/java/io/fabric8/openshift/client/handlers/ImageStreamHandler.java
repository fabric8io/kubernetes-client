...
public class ImageStreamHandler implements ResourceHandler<ImageStream> {
  ...
  @Override
  public Boolean delete(Client client, String namespace, ImageStream item) {
    try (OpenShiftClient osClient = client.adapt(OpenShiftClient.class)) {
      return new ImageStreamOperationsImpl(osClient, namespace, null, true, item).delete(item);
    }
  }
}
