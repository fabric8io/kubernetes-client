...
#### Bugs
* Fix #7896: (kubernetes-client) `AbstractWatchManager.watchEnded()` now resets `resourceVersion` to `null` when a watch closes cleanly with no messages within 2 seconds of opening, compensating for a GKE-specific behaviour where the GKFE proxy rejects stale `resourceVersion` on `v1/events` watches with a bare WebSocket close (code 1000, no body) instead of the standard `{"type":"ERROR","code":410}` — without this fix, `AbstractWatchManager` loops reconnecting with the same stale `resourceVersion` indefinitely
