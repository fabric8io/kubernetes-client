...
#### Bugs
* Fix #8024: (httpclient-vertx-5) WebSocket-over-TLS operations (`exec`/`attach`/`portForward`/WebSocket-backed watches) on a *derived* client - one produced by calling `newBuilder()`/`build()` on an already-built `HttpClient`, as `OpenShiftClientImpl` does internally on every `adapt(OpenShiftClient.class)` to install its OAuth token-refresh interceptor - once again trust the configured certificate instead of silently falling back to the JVM default trust store. `Vertx5HttpClientBuilder.build()`'s short-circuit path for derived clients reused the original client's plain HTTP connection (with its TLS config intact) but always created a brand-new `WebSocketClient` with bare Vert.x defaults, discarding the trust/key material entirely; this is why only WebSocket-based operations on an `OpenShiftClient`-adapted client were affected while regular REST calls worked fine. Fixes the follow-up gap left by #7907, whose fix only covered the non-derived build path
...
#### Bugs
* Fix #7954: (kubernetes-model-generator) OpenAPI model generation now validates enum extension values and neutralizes Java Unicode escape introducers in descriptions before rendering source, preventing schema-controlled enum and Javadoc text from injecting generated Java code
