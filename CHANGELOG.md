...
#### Bugs
* Fix #7956: (httpclient) `tlsServerName` now fails closed for HTTP clients that cannot enforce it and Jetty uses it as the TLS peer host as well as SNI, so certificate verification checks the configured Kubernetes API identity before credentials are sent
