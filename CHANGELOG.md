...
* Fix #7847: (sonar) miscellaneous reliability findings — S2445: `BaseClient.addToCloseable` now synchronizes on the internal `closable` set instead of the method parameter; S5998: `KubernetesResourceUtil.KUBERNETES_SUBDOMAIN_REGEX` uses possessive quantifiers to prevent ReDoS; S3064: `Serialization.yamlMapper()` uses local-variable DCL to prevent unsafe publication; S899: `MockWebServer` uses `Files.deleteIfExists()` for temp cert/key cleanup; S2637: `OkHttpClientBuilderImpl` guards against null `trustManager` before calling `sslSocketFactory()`; S2583: `OkHttpClientImpl.doClose()` removes dead null checks on dispatcher/connectionPool
