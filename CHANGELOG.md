...
#### Bugs
* Fix #7958: (kubernetes-client-api) Lower-priority `NO_PROXY` environment values no longer bypass proxies configured through JVM system properties. Proxy and bypass settings now retain their configuration-source priority, so `NO_PROXY` is applied only when it comes from a source at least as strong as the selected proxy; same-source combinations continue to work as before
