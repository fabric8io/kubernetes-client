...
* Fix #7091: The `crd2java` Gradle task is now compatible with the Gradle configuration cache. It holds only lazy properties (no `Project` or extension capture) and reads its CRD source roots at execution time, so the configuration-cache entry can be stored and reused across builds.
