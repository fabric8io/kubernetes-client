...
#### Bugs
* Fix #7957: (kubernetes-client-api) TLS trust configuration now honors the documented source precedence between CA material and the insecure trust-all flag. When kube config set `insecure-skip-tls-verify: true` (mapped to `trustCerts=true`) while a higher-priority source (system property or environment variable) supplied a CA certificate file or inline data, `SSLUtils` evaluated `isTrustCerts()` before the CA material and left trust-all in effect, silently ignoring the explicitly pinned CA and downgrading the connection to a TLS verification bypass. `Config.configFromSysPropsOrEnvVars` now resolves the CA file/data together with the trust-all and hostname-verification flags using source ranks (system property > environment variable > existing/kube config value) and clears the insecure flags when CA material comes from a strictly higher-priority source, so an explicitly configured CA is enforced instead of being overridden by a lower-priority insecure setting
