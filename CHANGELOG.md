...
#### Bugs
* Fix #7960: (kubernetes-client-api) Auto-configured auth now resolves competing bearer-token and basic credentials by source rank (system property > environment > kubeconfig). A higher-priority bearer token no longer leaves lower-priority kubeconfig basic credentials in place for `TokenRefreshInterceptor` to prefer, and a higher-priority basic pair clears a lower-priority token. Same-rank values keep the previous combined behaviour
